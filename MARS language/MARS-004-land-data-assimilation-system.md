...
will run daily with a 5-day spinup and will be intialized from the BRT product. Additionally,
...
statistis allows us to have the same layout under `class=od` and `rd`, and even for member state
data if required in the future. It avoids relying on specific `expver` ranges reserved for
research experiments, and facilitates a clean introduciton of the required MARS keywords under
the new streams.
...
confguration keywourd should be similarly dash-delineated. Using the `configuration` keyword in
...
### Related Decisions
 ...
 - [MARS-005 CEMS C3S Hydrology](MARS-005-CEMS-C3S-Hydrology.md) - This decisision is in
